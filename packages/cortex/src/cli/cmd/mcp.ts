    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in cortex.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .cortex/ subdirectory too)
  const candidates = [path.join(baseDir, "cortex.json"), path.join(baseDir, "cortex.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".cortex", "cortex.json"), path.join(baseDir, ".cortex", "cortex.jsonc"))
  }
  ...
  // Default to cortex.json if none exist
  return candidates[0]
}
